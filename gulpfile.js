...
gulp.task('generate-favicon', function(done) {
    realFavicon.generateFavicon({
        masterPicture: 'markup/static/favicon/favicon.svg',
        dest: 'build/',
        iconsPath: '/',
        design: {
            ios: {
            ...
            androidChrome: {
                ...
                themeColor: '#ffffff',
                manifest: {
                    name: JSON.parse(fs.readFileSync(pkgData)).description,
                    display: 'standalone',
                    orientation: 'notSet',
                    onConflict: 'override',
                    declared: true
...
gulp.task('css:blocks', function () {
    return combiner(
        ...
        sourcemaps.init(),
        gulpIf(path.basename(global.stylusChangedFile) !== 'variables.styl' && path.basename(global.stylusChangedFile) !== 'mixins.styl', cache('stylus')),
...
gulp.task('watch', function() {
    ...
    gulp.watch(['app_components/bootstrap/css/variables.styl', 'app_components/bootstrap/css/mixins.styl', 'markup/components/**/*.styl'], gulp.series('css:blocks', function reloadCSS_blocks(done) {
        ...
        if (event === 'deleted') { // if a file is deleted, forget about it
            delete cache.caches['stylus'][path.resolve(filepath)];
            remember.forget('stylus', path.resolve(filepath));
        }
        global.stylusChangedFile = path.resolve(filepath); //console.log(event);console.log(event.type);console.log(event.path);console.log(filepath);
    });
